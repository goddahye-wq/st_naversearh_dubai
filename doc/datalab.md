...
### 세분화된 정보 확인
설정한 주제군에 대해 기간을 일간, 주간, 월간 단위로 조회할 수 있습니다. 5살 단위 연령, 성별, 기기별(PC/모바일) 구분이 가능합니다.

### 상대적 값 제공
요청 기간 중 검색 횟수가 가장 높은 시점을 100으로 둔 상대적 값으로 제공됩니다.

---

## 2. 쇼핑인사이트
쇼핑인사이트는 네이버쇼핑 서비스에서 사용자가 클릭한 데이터를 조회할 수 있는 API입니다. 쇼핑 분야별, 연령별, 성별, 기기별로 세분화된 클릭 트렌드를 제공합니다.

### 분야별 통계
네이버쇼핑의 4단계 카테고리(카테고리 1 > 2 > 3 > 4) 중심의 통계 정보를 제공합니다.

### 검색어 클릭 추이
지정한 분야의 상품 검색 결과를 사용자가 얼마나 많이 클릭했는지 추이를 확인할 수 있습니다.

### 주요 구매층 파악
연령별, 성별, 기기별 클릭 비중과 추이를 파악할 수 있습니다. 분야 조회 시 `cat_id` 파라미터를 사용합니다.

### 검색어 통계
특정 쇼핑 분야에서 발생한 검색어의 세부 통계를 제공하여 마케팅 타깃 설정에 활용할 수 있습니다.
- 쇼핑인사이트 값 역시 최대 클릭 횟수를 100으로 둔 상대적 값으로 제공됩니다.
...
분야별 통계를 확인할 수 있습니다 
네이버쇼핑 서비스에는 카테고리 1 > 카테고리 2 > 카테고리 3 > 카테고리 4와 같이 4단계의 수준으로 나뉘는 쇼핑 분야가 있습니다. 쇼핑 분야 중심의 세부 통계 정보를 제공해 소상공인의 재고 관리, 마케팅 등에 활용할 수 있게 합니다.
