# 네이버 오픈 API 가이드 (비로그인 방식)

## 1. 비로그인 방식 오픈 API란?
HTTP 요청 헤더에 클라이언트 아이디와 클라이언트 시크릿 값만 전송하여 사용할 수 있는 API입니다. 네이버 로그인 인증을 통한 접근 토큰(Access Token) 획득이 필요 없습니다.
...
- **지식iN**: `https://openapi.naver.com/v1/search/kin`
- **카페글**: `https://openapi.naver.com/v1/search/cafearticle`

### 기타
- **캡차 (Captcha)**: 이미지/음성 캡차 기능 제공
- **Clova Face Recognition**: 얼굴 인식 및 유명인 닮은꼴 찾기

## 3. 공통 특징
- **인증**: `X-Naver-Client-Id`, `X-Naver-Client-Secret` 헤더 사용
- **호출 방식**: 각 API별 레퍼런스에 따른 GET 또는 POST 요청
...
캘린더: 외부 서비스에 네이버 캘린더에 일정을 등록할 수 있게 하는 API입니다.
네이버 로그인 
