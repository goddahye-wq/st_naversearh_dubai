...
## 1. 개요
쇼핑인사이트 API는 네이버 데이터랩의 쇼핑인사이트 데이터를 프로그래밍 방식으로 조회할 수 있는 RESTful API입니다. 네이버 통합검색 및 네이버쇼핑에서의 검색 클릭 추이 데이터를 JSON 형식으로 반환합니다.

- **방식**: 비로그인 방식 오픈 API (Client ID/Secret 필요)
- **한도**: 하루 1,000회
- **반환 형식**: JSON
...
- **주요 파라미터**:
    ...
    - `category`: 조회할 카테고리 설정 (name, param: [cat_id])
    - `device`: 기기별 (pc, mo)
    - `gender`: 성별 (m, f)
    - `ages`: 연령대 (10, 20, 30, 40, 50, 60)

## 3. 키워드별 트렌드 조회
특정 쇼핑 분야 내에서 검색 키워드별 검색 클릭 추이를 조회합니다.

- **URL**: `https://openapi.naver.com/v1/datalab/shopping/category/keywords`
- **HTTP 메서드**: POST
- **주요 파라미터**:
    - `category`: 대상 카테고리 ID
    - `keyword`: 조회할 키워드 설정 (name, param: [키워드])
    - 기타 조건(startDate, endDate, device 등)은 분야별 조회와 동일
...
사용 API에서 데이터랩 (쇼핑인사이트)를 선택해 추가합니다.
